<template>
    <WorkspaceLayout title="Subcategories">
        <div class="mx-auto">
            <div class="mb-6 flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        Manage subcategories under each course category.
                    </p>
                </div>
                <button
                    @click="openCreate"
...
const openEdit = (subcategory) => {
    ...
    form.sort_order = subcategory.sort_order ?? 0;
    showModal.value = true;
};
...
const submit = () => {
    if (editingSubcategory.value) {
        form.put(`/lms/subcategories/${editingSubcategory.value.id}`, {
            preserveScroll: true,
            onSuccess: closeModal,
        });
    } else {
        form.post('/lms/subcategories', {
            preserveScroll: true,
            onSuccess: closeModal,
        });
    }
...
const destroy = (subcategory) => {
    if (!confirm(`Delete subcategory "${subcategory.name}"?`)) return;

    router.delete(`/lms/subcategories/${subcategory.id}`, {
        preserveScroll: true,
    });
};
</script>
