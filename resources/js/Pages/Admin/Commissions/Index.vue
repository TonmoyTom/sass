    ...
    <AdminLayout title="Commissions">
        <div class="mx-auto">
            ...
            <div class="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-5">
                ...
                <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
                    <p class="text-xs text-gray-500 dark:text-gray-400">Approved Amount</p>
                    <p class="mt-1 text-2xl font-bold text-green-600 dark:text-green-400">
                        ৳{{ money(totals.approved) }}
                    </p>
                </div>
                <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
                    <p class="text-xs text-gray-500 dark:text-gray-400">Pending Count</p>
            ...
            <DataTable
                :data="commissions"
                :filters="filters"
                route-name="admin.commissions.index"
                :columns="[
                    { key: 'seller', label: 'Seller' },
                    { key: 'module_tenant', label: 'Module / Tenant' },
            ...
            >
                ...
                <template #row="{ row: c }">
                    <td class="px-5 py-4">
                        <p class="font-medium text-gray-800 dark:text-white/90">{{ c.seller_name }}</p>
                        <p class="text-xs text-gray-500 dark:text-gray-400">{{ c.seller_email }}</p>
                        <p class="text-xs text-gray-500 dark:text-gray-400">{{ c.created_at }}</p>
                    </td>

                    <td class="px-5 py-4">
                        <p class="text-sm text-gray-800 dark:text-white/90">{{ c.module_name }}</p>
                        <p class="text-xs text-gray-500 dark:text-gray-400">{{ c.tenant_name }}</p>
                        <p class="text-xs text-gray-400">{{ c.tenant_email }}</p>
                    </td>
                    <td class="px-5 py-4">
                        <p class="font-semibold text-gray-800 dark:text-white/90">৳{{ money(c.amount) }}</p>
                        <p class="text-xs text-gray-500 dark:text-gray-400">{{ c.rate }}% · {{ c.type }}</p>
                        <p v-if="c.is_held" class="mt-0.5 text-xs text-amber-600 dark:text-amber-400">
...
const approve = (c) => {
    router.post(route('admin.commissions.approve', c.id), {}, { preserveScroll: true });
};
