...
// limits object → _limitPairs array (backend → form)
const limitsToPairs = (limits) => {
    const pairs = Object.entries(limits || {}).map(([key, value]) => ({
        key,
        value,
    }));
...
const form = useForm({
    ...
    version: props.module.version ?? '1.0.0',
    pricing_type: props.module.pricing_type,
    commission_rate: props.module.commission_rate,
    module_category: props.module.module_category,
    is_active: props.module.is_active,
    is_core: props.module.is_core,
    sort_order: props.module.sort_order,
    features: props.module.features ?? '',
    tiers: (props.module.tiers ?? []).map((t) => ({
        id: t.id,
        name: t.name,
        slug: t.slug,
        ...
        _limitPairs: limitsToPairs(t.limits),
    })),
});

// jodi kono tier na thake, ekta empty
if (!form.tiers.length) form.tiers.push(makeTier());
