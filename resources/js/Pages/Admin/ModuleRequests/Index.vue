    ...
    <AdminLayout title="Module Requests">
        ...
        >
            ...
            <DataTable
                :data="requests"
                :filters="filters"
                route-name="admin.module-requests.index"
                :columns="[
            ...
            >
                <template #filters="{ filters: f, apply }">
                    <div class="flex flex-wrap gap-1 rounded-lg bg-gray-100 p-1 dark:bg-white/[0.03]">
                        <button
                ...
                <template #row="{ row: req }">
                    ...
                    <td class="px-4 py-3">
                        ...
                        <span class="font-mono text-xs text-gray-500 dark:text-gray-400">{{ req.module_alias }}</span>
                    </td>
                    <td class="px-4 py-3 text-gray-600 dark:text-gray-400">
                        {{ req.commission }}%
                    </td>
                    <td class="max-w-[200px] px-4 py-3 text-gray-600 dark:text-gray-400">
                        <span class="line-clamp-2 text-xs">{{ req.note || '—' }}</span>
                        <span v-if="req.admin_note" class="mt-1 block text-xs text-red-500">Admin: {{ req.admin_note }}</span>
                    </td>
                    <td class="px-4 py-3">
                        <span
                    ...
                    <td class="px-4 py-3 text-xs text-gray-500 dark:text-gray-400">
                        <template v-if="req.reviewed_at">
                            {{ req.reviewed_by }}<br />{{ req.reviewed_at }}
                        </template>
                        <span v-else>—</span>
                    </td>
                    <td class="px-4 py-3 text-right">
                        <div class="flex items-center justify-end gap-2">
                            <Link
                                :href="route('admin.module-requests.show', req.id)"
