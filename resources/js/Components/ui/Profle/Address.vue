                        ...
                        <div class="custom-scrollbar overflow-y-auto px-2">
                            ...
                            >
                                ...
                                <FormInput
                                    v-model="form.tx_id"
                                    label="TAX ID"
                                    :error="form.errors.tx_id"
                                />
                            </div>
                        </div>

                        <div
                            class="mt-6 flex items-center gap-3 lg:justify-end"
                        >
                            <button
                                @click="isModal = false"
                                type="button"
...
const form = useForm({
    ...
    tx_id: props.user?.info?.tx_id ?? '',
});

const save = () => {
    form.patch(route('profile.address.update'), {
        preserveScroll: true,
        onSuccess: () => (isModal.value = false),
    });
};
</script>
