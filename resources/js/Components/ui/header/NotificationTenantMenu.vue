...
const tenantId = page.props.workspace.tenant?.id;
...
const handleItemClick = async (notification) => {
    if (!notification.read) {
        await fetch(`/notifications/${notification.id}/read`, {
            ...
            credentials: 'include',
            body: JSON.stringify({ source: notification.source }),
        });
...
const loadNotifications = async () => {
    try {
        ...
        // central + tenant merged response, backend theke 'source' + 'user' soho ashe
        notifications.value = data.map((n) => ({
            id: n.id,
            source: n.source ?? 'central',
...
// central channel er sathe collision r fix er por, ekhane consistent channel name use kora hocche
const getChannelName = (userId) => `tenant.${tenantId}.user.${userId}`;
...
const setupEchoListener = () => {
    ...
    if (!tenantId) {
        console.warn('Tenant ID not found in page props');
        return;
    }
    ...
    // age kono leftover subscription thakle sheta thik channel name diye leave kora
