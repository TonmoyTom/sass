...
class ModuleRequestController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        $requests = SellerModuleRequest::query()
            ...
            ->filterAndCache(
                $request,
                searchable: [
                    'seller.user.name',    // nested relation — seller → user → name
                    'seller.user.email',
                    'module.name',
                ],
                filterable: ['status'],
                sortable: ['created_at', 'reviewed_at'],
                ttlSeconds: 300,
                perPage: 15,
                transform: fn ($r) => [
                    'id' => $r->id,
    ...
    {
        $moduleRequest->load(['seller.user', 'module']);

        $sellerUserId = $moduleRequest->seller?->user_id;
        $moduleName = $moduleRequest->module?->name ?? 'module';
        $reason = $moduleRequest->admin_note
                        ? " Reason: {$moduleRequest->admin_note}"
                        : '';
        ...
        NotificationSent::dispatch(
            "Your request to sell the \"{$moduleName}\" module has been approved. You can now start selling it.",
            $sellerUserId,
            'success',
            '/seller/modules'
        );
    ...
    public function reject(Request $request, SellerModuleRequest $moduleRequest): RedirectResponse
    {
        ...
        NotificationSent::dispatch(
            "Your request to sell the \"{$moduleName}\" module has been rejected.{$reason}",
            $sellerUserId,
            'error',
            '/seller/modules'
        );
    ...
    public function updateStatus(Request $request, SellerModuleRequest $moduleRequest): RedirectResponse
    {
        ...
        // seller ke notify (pending e ferot nile notify dorkar nai)
    ...
    public function updateNote(Request $request, SellerModuleRequest $moduleRequest): RedirectResponse
    {
        ...
        $oldNote = $moduleRequest->admin_note;

        $moduleRequest->update([
            'admin_note' => $data['admin_note'] ?? null,
        ]);

        // notun/changed note thakle e seller ke janaw (empty korle na)
        $newNote = $data['admin_note'] ?? null;

        if ($newNote && $newNote !== $oldNote) {
            $moduleRequest->load(['seller', 'module']);
            ...
            if ($sellerUserId) {
                NotificationSent::dispatch(
                    "Admin added a note to your \"{$moduleName}\" module request: {$newNote}",
                    $sellerUserId,
                    'info',
                    '/seller/modules'
                );
